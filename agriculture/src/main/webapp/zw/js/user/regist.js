...
function validateLoginname() {
	 ...
	 */
	 $.ajax({
	        url:"/AgricultureSale0.1/user/AjaxValidateUsername.action",
	        data:{username:value},
	        type:"post",
	        dataType:"json",
	        async:false,
...
function validateEmail() {
	 ...
	 */
	$.ajax({
        url:"/AgricultureSale0.1/user/AjaxValidateEmail.action",
        data:{email:value},
        type:"post",
        dataType:"json",
        async:false,
...
function validateVerifyCode() {
	 ...
	 */
	$.ajax({
		url:"/AgricultureSale0.1/verify/AjaxValidateVerifyCode.action",
 ...
 */
function _hyz() {
	$("#imgVerifyCode").attr("src","/AgricultureSale0.1/verify/VerifyCode.action?a=" + new Date().getTime());//不加参数的话，验证码不会改变，跟游览器的缓存有关
}
/*
 * 实现光标进入注册图标的时候换一张图片
 */
function replaceRegistImg() {
	$("#submitBtn").hover(
      function(){
		$("#submitBtn").attr("src","/AgricultureSale0.1/zw/image/regist2.jpg");
	  },
	  function(){
		$("#submitBtn").attr("src","/AgricultureSale0.1/zw/image/regist1.jpg");  
	  }
	);
}
